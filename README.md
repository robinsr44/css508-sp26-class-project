...
| **Browser E2E (Playwright)** | Start **`moon-api` on 8080** (same as UI dev). From `src/frontend`: **once** run `npm run test:e2e:install` to download Chromium; then `npm run test:e2e` (starts Vite on **5173** and hits live `/api` via proxy). See [`E2ETestPlan.md`](E2ETestPlan.md). |
