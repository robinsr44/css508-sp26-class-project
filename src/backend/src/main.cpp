...
static nlohmann::json build_sun_json(const moon::SunResult& r, int y, int m, int d, int hh, int mm, double lat,
                                     double lon) {
  nlohmann::json j;
  char instant[40];
  std::snprintf(instant, sizeof(instant), "%04d-%02d-%02dT%02d:%02d:00Z", y, m, d, hh, mm);
...
static void handle_sun(double lat, double lon, int y, int m, int d, int hh, int mm, httplib::Response& res) {
  if (lat < -90.0 || lat > 90.0 || lon < -180.0 || lon > 180.0) {
    res.status = 400;
    res.set_content(R"({"error":"lat must be in [-90,90] and lon in [-180,180]"})", "application/json");
    set_cors(res);
...
int main(int argc, char** argv) {
  ...
  svr.Post("/api/sun", [](const httplib::Request& req, httplib::Response& res) {
    ...
    const double lat = body["lat"].get<double>();
    const double lon = body["lon"].get<double>();
    const std::string date_str = body["date"].get<std::string>();
    int y = 0, m = 0, dday = 0;
    if (!moon::parse_date(date_str, y, m, dday)) {
      res.status = 400;
    ...
    handle_sun(lat, lon, y, m, dday, hh, mm, res);
  });
